...
/**
 * In dev mode on loopback, infer the server WebSocket URL from the current
 * page port. The dev-runner allocates server and web ports with a fixed gap
 * (e.g. web=5733, server=3773). When VITE_WS_URL is not set (standalone
 * `bun run dev:web`, desktop mode, etc.) we can still find the server.
 */
