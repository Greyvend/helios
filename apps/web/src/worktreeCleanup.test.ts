import { ProjectId, ThreadId } from "@helios-dev/contracts";
...
describe("formatWorktreePathForDisplay", () => {
  it("shows only the last path segment for unix-like paths", () => {
    const result = formatWorktreePathForDisplay(
      "/Users/julius/.helios/worktrees/helios-mvp/helios-4e609bb8",
    );
    expect(result).toBe("helios-4e609bb8");
  });

  it("normalizes windows separators before selecting the final segment", () => {
    const result = formatWorktreePathForDisplay(
      "C:\\Users\\julius\\.t3\\worktrees\\helios-mvp\\helios-4e609bb8",
    );
    expect(result).toBe("helios-4e609bb8");
  });

  it("uses the final segment even when outside ~/.helios/worktrees", () => {
