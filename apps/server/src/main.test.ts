...
import type { OrchestrationReadModel } from "@helios-dev/contracts";
...
import { CliConfig, recordStartupHeartbeat, heliosCli, type CliConfigShape } from "./main";
...
const runCli = (
  ...
  env: Record<string, string> = { HELIOS_NO_BROWSER: "true" },
) => {
  return Command.runWith(heliosCli, { version: "0.0.0-test" })(args).pipe(
    Effect.provide(
...
it.layer(testLayer)("server CLI command", (it) => {
  ...
  it.effect("uses env fallbacks when flags are not provided", () =>
    Effect.gen(function* () {
      yield* runCli([], {
        HELIOS_MODE: "desktop",
        HELIOS_PORT: "4999",
        HELIOS_HOST: "100.88.10.4",
        HELIOS_HOME: "/tmp/t3-env-home",
        VITE_DEV_SERVER_URL: "http://localhost:5173",
        HELIOS_NO_BROWSER: "true",
        HELIOS_AUTH_TOKEN: "env-token",
      });
  ...
  it.effect("applies CLI then env precedence over bootstrap envelope values", () =>
    Effect.gen(function* () {
      ...
      yield* runCli(["--port", "4999", "--host", "0.0.0.0", "--auth-token", "cli-token"], {
        ...
        HELIOS_HOME: "/tmp/t3-env-home",
        HELIOS_NO_BROWSER: "true",
        HELIOS_AUTO_BOOTSTRAP_PROJECT_FROM_CWD: "true",
        HELIOS_LOG_WS_EVENTS: "true",
      });
  ...
  it.effect("prefers --mode over HELIOS_MODE", () =>
    Effect.gen(function* () {
  ...
  it.effect("prefers --no-browser over HELIOS_NO_BROWSER", () =>
    Effect.gen(function* () {
      yield* runCli(["--no-browser"], {
        HELIOS_NO_BROWSER: "false",
      });
  ...
  it.effect("supports CLI and env for bootstrap/log websocket toggles", () =>
    Effect.gen(function* () {
      yield* runCli(["--auto-bootstrap-project-from-cwd"], {
        HELIOS_MODE: "desktop",
        HELIOS_LOG_WS_EVENTS: "false",
        HELIOS_AUTO_BOOTSTRAP_PROJECT_FROM_CWD: "false",
        HELIOS_NO_BROWSER: "true",
      });
