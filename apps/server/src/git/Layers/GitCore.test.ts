  ...
  describe("renameGitBranch", () => {
    ...
    it.effect("appends numeric suffix when target branch already exists", () =>
      Effect.gen(function* () {
        const tmp = yield* makeTmpDir();
        yield* initRepoWithCommit(tmp);
        yield* (yield* GitCore).createBranch({ cwd: tmp, branch: "helios/feat/session" });
        yield* (yield* GitCore).createBranch({ cwd: tmp, branch: "helios/tmp-working" });
        yield* (yield* GitCore).checkoutBranch({ cwd: tmp, branch: "helios/tmp-working" });
    ...
    it.effect("increments suffix until it finds an available branch name", () =>
      Effect.gen(function* () {
        ...
        yield* initRepoWithCommit(tmp);
        yield* (yield* GitCore).createBranch({ cwd: tmp, branch: "helios/feat/session" });
        yield* (yield* GitCore).createBranch({ cwd: tmp, branch: "helios/feat/session-1" });
        yield* (yield* GitCore).createBranch({ cwd: tmp, branch: "helios/tmp-working" });
        yield* (yield* GitCore).checkoutBranch({ cwd: tmp, branch: "helios/tmp-working" });
