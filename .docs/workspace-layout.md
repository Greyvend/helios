...
- `/apps/desktop`: Electron shell. Spawns a desktop-scoped `helios` backend process and loads the shared web app.
...
- `/packages/shared`: Shared runtime utilities consumed by both server and web. Uses explicit subpath exports (e.g. `@helios-dev/shared/git`, `@helios-dev/shared/DrainableWorker`) — no barrel index.
