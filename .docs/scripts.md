...
- Dev commands default `HELIOS_STATE_DIR` to `~/.helios/dev` to keep dev state isolated from desktop/prod state.
- Override server CLI-equivalent flags from root dev commands with `--`, for example:
  `bun run dev -- --state-dir ~/.helios/another-dev-state`
...
Set `HELIOS_DEV_INSTANCE` to any value to deterministically shift all dev ports together.

- Default ports: server `3773`, web `5733`
- Shifted ports: `base + offset` (offset is hashed from `HELIOS_DEV_INSTANCE`)
- Example: `HELIOS_DEV_INSTANCE=branch-a bun run dev:desktop`

If you want full control instead of hashing, set `HELIOS_PORT_OFFSET` to a numeric offset.
