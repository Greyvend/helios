...
import { NetService } from "@helios-dev/shared/Net";
...
const OffsetConfig = Config.all({
  portOffset: optionalIntegerConfig("HELIOS_PORT_OFFSET"),
  devInstance: optionalStringConfig("HELIOS_DEV_INSTANCE"),
});
...
export function resolveOffset(config: {
  ...
  if (config.portOffset !== undefined) {
    if (config.portOffset < 0) {
      throw new Error(`Invalid HELIOS_PORT_OFFSET: ${config.portOffset}`);
    }
    return {
      offset: config.portOffset,
      source: `HELIOS_PORT_OFFSET=${config.portOffset}`,
    };
  }
  ...
  if (/^\d+$/.test(seed)) {
    return { offset: Number(seed), source: `numeric HELIOS_DEV_INSTANCE=${seed}` };
  }

  const offset = ((Hash.string(seed) >>> 0) % MAX_HASH_OFFSET) + 1;
  return { offset, source: `hashed HELIOS_DEV_INSTANCE=${seed}` };
}
...
export function createDevRunnerEnv({
  ...
  return Effect.gen(function* () {
    ...
    const output: NodeJS.ProcessEnv = {
      ...
      HELIOS_STATE_DIR: resolvedStateDir,
    };
    ...
    if (autoBootstrapProjectFromCwd !== undefined) {
      output.HELIOS_AUTO_BOOTSTRAP_PROJECT_FROM_CWD = autoBootstrapProjectFromCwd ? "1" : "0";
    } else {
      delete output.HELIOS_AUTO_BOOTSTRAP_PROJECT_FROM_CWD;
    }

    if (logWebSocketEvents !== undefined) {
      output.HELIOS_LOG_WS_EVENTS = logWebSocketEvents ? "1" : "0";
    } else {
      delete output.HELIOS_LOG_WS_EVENTS;
    }
  ...
  return Effect.gen(function* () {
    const { portOffset, devInstance } = yield* OffsetConfig.asEffect().pipe(
      Effect.mapError(
        (cause) =>
          new DevRunnerError({
            message: "Failed to read HELIOS_PORT_OFFSET/HELIOS_DEV_INSTANCE configuration.",
            cause,
          }),
      ),
    );
    ...
    const envOverrides = {
      noBrowser: readOptionalBooleanEnv("HELIOS_NO_BROWSER"),
      autoBootstrapProjectFromCwd: readOptionalBooleanEnv("HELIOS_AUTO_BOOTSTRAP_PROJECT_FROM_CWD"),
      logWebSocketEvents: readOptionalBooleanEnv("HELIOS_LOG_WS_EVENTS"),
    };
    ...
    yield* Effect.logInfo(
      `[dev-runner] mode=${input.mode} source=${source}${selectionSuffix} serverPort=${String(env.HELIOS_PORT)} webPort=${String(env.PORT)} stateDir=${String(env.HELIOS_STATE_DIR)}`,
    );
...
const devRunnerCli = Command.make("dev-runner", {
  ...
  stateDir: Flag.string("state-dir").pipe(
    Flag.withDescription("State directory path (forwards to HELIOS_STATE_DIR)."),
    Flag.withFallbackConfig(optionalStringConfig("HELIOS_STATE_DIR")),
  ),
  authToken: Flag.string("auth-token").pipe(
    Flag.withDescription("Auth token (forwards to HELIOS_AUTH_TOKEN)."),
    Flag.withAlias("token"),
    Flag.withFallbackConfig(optionalStringConfig("HELIOS_AUTH_TOKEN")),
  ),
  noBrowser: Flag.boolean("no-browser").pipe(
    Flag.withDescription("Browser auto-open toggle (equivalent to HELIOS_NO_BROWSER)."),
    Flag.withFallbackConfig(optionalBooleanConfig("HELIOS_NO_BROWSER")),
  ),
  autoBootstrapProjectFromCwd: Flag.boolean("auto-bootstrap-project-from-cwd").pipe(
    Flag.withDescription(
      "Auto-bootstrap toggle (equivalent to HELIOS_AUTO_BOOTSTRAP_PROJECT_FROM_CWD).",
    ),
    Flag.withFallbackConfig(optionalBooleanConfig("HELIOS_AUTO_BOOTSTRAP_PROJECT_FROM_CWD")),
  ),
  logWebSocketEvents: Flag.boolean("log-websocket-events").pipe(
    Flag.withDescription("WebSocket event logging toggle (equivalent to HELIOS_LOG_WS_EVENTS)."),
    Flag.withAlias("log-ws-events"),
    Flag.withFallbackConfig(optionalBooleanConfig("HELIOS_LOG_WS_EVENTS")),
  ),
  host: Flag.string("host").pipe(
    Flag.withDescription("Server host/interface override (forwards to HELIOS_HOST)."),
    Flag.withFallbackConfig(optionalStringConfig("HELIOS_HOST")),
  ),
  port: Flag.integer("port").pipe(
    Flag.withSchema(Schema.Int.check(Schema.isBetween({ minimum: 1, maximum: 65535 }))),
    Flag.withDescription("Server port override (forwards to HELIOS_PORT)."),
    Flag.withFallbackConfig(optionalPortConfig("HELIOS_PORT")),
  ),
