...
it.layer(NodeServices.layer)("dev-runner", (it) => {
  describe("resolveOffset", () => {
    it.effect("uses explicit HELIOS_PORT_OFFSET when provided", () =>
      Effect.sync(() => {
        const result = resolveOffset({ portOffset: 12, devInstance: undefined });
        assert.deepStrictEqual(result, {
          offset: 12,
          source: "HELIOS_PORT_OFFSET=12",
        });
      }),
    );
    ...
    it.effect("throws for negative port offset", () =>
      Effect.gen(function* () {
        ...
        assert.ok(error.includes("Invalid HELIOS_PORT_OFFSET"));
      }),
    );
  });
  ...
  describe("createDevRunnerEnv", () => {
    ...
    it.effect("supports explicit typed overrides", () =>
      Effect.gen(function* () {
        ...
        assert.equal(env.HELIOS_AUTO_BOOTSTRAP_PROJECT_FROM_CWD, "0");
        assert.equal(env.HELIOS_LOG_WS_EVENTS, "1");
        assert.equal(env.HELIOS_HOST, "0.0.0.0");
    ...
    it.effect("does not force websocket logging on in dev mode when unset", () =>
      Effect.gen(function* () {
        const env = yield* createDevRunnerEnv({
          mode: "dev",
          baseEnv: {
            HELIOS_LOG_WS_EVENTS: "keep-me-out",
          },
          serverOffset: 0,
          webOffset: 0,
          stateDir: undefined,
        ...
        assert.equal(env.HELIOS_MODE, "web");
        assert.equal(env.HELIOS_LOG_WS_EVENTS, undefined);
      }),
    );
